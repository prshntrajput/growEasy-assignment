...
export class ImportController {
  public static async parseCsvFile(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const { headers, rows } = csvParserService.parseBuffer(req.file.buffer);

      sendSuccess(res, { headers, rows }, 200, 'CSV parsed successfully');
    } catch (err) {
      next(err);
    }
  }
  ...
  public static async confirmImport(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      sendSuccess(
        res,
        { jobId, totalBatches: batches.length, totalRows: normalizedRows.length },
        202,
