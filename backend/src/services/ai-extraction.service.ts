...
export class AiExtractionService {
  ...
  private sanitizeEnumFields(record: AiExtractedRecord): AiExtractedRecord {
    ...
    if (
      ...
      !VALID_CRM_STATUSES.has(sanitized.crm_status)
    ) {
      console.warn(
        `Invalid crm_status "${sanitized.crm_status}" received, clearing to blank`
      );
      sanitized.crm_status = '';
    }
    ...
    if (
      ...
      !VALID_DATA_SOURCES.has(sanitized.data_source)
    ) {
      console.warn(
        `Invalid data_source "${sanitized.data_source}" received, clearing to blank`
      );
      sanitized.data_source = '';
    }
